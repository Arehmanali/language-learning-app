...
export const ExamScreen: React.FC<ExamProps> = (props) => {
  ...
  React.useEffect(() => {
    ...
    const gap: DeEnPair | undefined = currProblem.problem.find(
      (item: DeEnPair) => item.gap == true,
    );
    if (gap)
      setGapInfor({
        ...gapInfor,
    ...
    else console.error("There is error on problem. Can't find gap.");
  }, [currProblem]);
  ...
  const onPressContinue = () => {
    ...
    if (gapInfor.text != '') {
      if (gapInfor.state != GapState.NONE) {
        // load next problem
      ...
      if (gap == undefined)
        ...
        return;
      if (gapInfor.text == gap.de) {
        // use select correct answer
        setGapInfor({
          ...gapInfor,
  ...
  const Example = (props: any) => {
    if (!currProblem) return <View style={styles.exampleContainer} />;
    ...
    const textList = wordList.map((w: string) => {
      if (w.startsWith('[[') == true)
        return (
          <Text key={w} style={styles.boldUnderline} category="h3" status="basic">
            {w.replace(/\[|\]/g, '')}
          </Text>
        );
      else
        return (
          <Text key={w} style={styles.example} category="h3" status="basic">
            {w.trim()}
          </Text>
        );
    });
    return <View style={styles.exampleContainer}>{textList}</View>;
  };

  const Problem = (props: any) => {
    if (currProblem == null) return <View style={styles.problem} />;
    const wordList = currProblem.problem.map((item: DeEnPair, index: number) => {
      if (item.gap == true)
        return (
          <GapText key={item.de + index} text={gapInfor.text} state={gapInfor.state} />
        );
      return (
        <DeText
          key={item.de + index}
    ...
    return <View style={styles.problem}>{wordList}</View>;
  };

  const Suggest = (props: any) => {
    if (currProblem == null) return <View style={styles.suggest} />;
    ...
    return <View style={styles.suggest}>{suggestList}</View>;
  };

  const getActionPadState = () => {
    if (gapInfor.text == '') return ActionPadState.NONE;
    else if (gapInfor.state == GapState.NONE) return ActionPadState.READY;
    else if (gapInfor.state == GapState.CORRECT) return ActionPadState.CORRECT;
      ...
      {finished == false ? (
        ...
        <Layout style={styles.board}>
          ...
          <Text category="h1" style={styles.endScore}>
            Result score:{' '}
            {Math.round((correctCount / (problems ? problems.length : 1)) * 100)}
          </Text>
          <TouchableOpacity style={styles.button} onPress={() => onPressTryAgain()}>
