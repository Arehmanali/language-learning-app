...
export const ActionPad = ({ label, state, answer, onPress }: ActionPadProps) => {
  ...
  else if (state == ActionPadState.CORRECT) containerColors = ['#08DAE8', '#45E9E8'];
  else if (state == ActionPadState.WRONG) containerColors = ['#F97789', '#FF938E'];

  let buttonColors: string[] = ['transparent', 'transparent'];
  if (state == ActionPadState.NONE) buttonColors = ['transparent', 'transparent'];
  else if (state == ActionPadState.READY) buttonColors = ['#08DAE8', '#45E9E8'];
  else buttonColors = ['#FFFFFE', '#FFFFFD'];
  let buttonTextStyle = {
    ...styles.buttonText,
  ...
  return (
    <LinearGradient colors={containerColors} style={styles.container}>
      <View style={styles.descContainer}>
        {state == ActionPadState.CORRECT && (
          <Text style={styles.descText}>Greate Job!</Text>
        )}
        {state == ActionPadState.WRONG && (
          <Text style={styles.descText}>
            Answer: <Text style={styles.answerText}>{answer}</Text>
          </Text>
        )}
        {(state == ActionPadState.CORRECT || state == ActionPadState.WRONG) && (
